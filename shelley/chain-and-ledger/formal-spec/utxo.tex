...
\begin{figure}[htb]
  \begin{align*}
    ...
    \fun{allCertValidate}~{utxo}~{tx} + \fun{allWDRLSValidate}~{utxo}~{tx} \\
...
* the corresponding
full validator is looked up (by hash value) in the finite map $\fun{txvals}$ in the
...
\begin{figure}[htb]
  \begin{align*}
    ...
    &~~\forall~h\mapsto (\var{script_v},\var{exunits})\in \fun{regCred}~\var{cert}\restrictdom\fun{txvals}~{tx}, \\
    ...
    & ~~  \vert \fun{regCred}~\var{cert}\restrictdom\fun{txvals}~{tx} \vert = 1 \\
    %
    \nextdef
    ...
    & ~~ \forall~ h\mapsto (\var{script_v},\var{exunits})\in \fun{validatorHash}~\var{a}\restrictdom\fun{txvals}~{tx}, \\
    & ~~ \llbracket \var{script_v} \rrbracket_{exunits}
    \fun{toData}(\fun{validationData}~\var{utxo}~\var{tx}~
      (h,\fun{adaVal}~c))) \\
    & ~~ \wedge ~ h = ~\fun{hashScript}~\var{script_v}  ~\wedge~ \\
    & ~~  \vert \fun{validatorHash}~\var{a}\restrictdom\fun{txvals}~{tx} \vert = 1 \\
    %
    \nextdef
    ...
    & ~~\forall ~h\mapsto \var{script_v}\in \fun{validatorHash}~\var{a}\restrictdom\fun{txvals}~{tx}, \\
    & ~~ \forall~(txid,ix) \mapsto ((a,v),hashscr_d) \in \var{(txid,ix)}\restrictdom\var{utxo}, \\
    & ~~ \var{hashscr_d} = \fun{hashData}~\var{script_d} ~\wedge~ \\
    & ~~ \llbracket \var{script_v} \rrbracket_{exunits} (\var{script_d},\var{script_r},
    \fun{toData}(\fun{validationData}~\var{utxo}~\var{tx}~
      ((txid,ix),h,\fun{hashData}~{script_r},v))) \\
    &~~ \wedge~ h = ~\fun{hashScript}~\var{script_v}  ~\wedge~ \\
    & ~~  \vert \fun{validatorHash}~\var{a}\restrictdom\fun{txvals}~{tx} \vert = 1) ~\wedge~ \\~\\
      %
    & ~~(\forall~(txid,ix,(a,v)) \in \fun{txinputs_s}~\var{tx}, \\
    & ~~\forall ~h\mapsto \var{script_v}\in \fun{validatorHash}~\var{a}\restrictdom\fun{txvals}~{tx}, \\
