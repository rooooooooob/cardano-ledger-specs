...
\begin{figure}[htb]
  \begin{align*}
    ...
    & ~~ \fun{validationData}~\var{utxo}~\var{tx}~
      (txid,ix,(\var{hash_d},\var{hash_r})))) ~\vert \\
    & ~~(txid,ix,(\var{hash_d},\var{hash_r})) \in
    \fun{txinputs_s}~\var{tx}, \\
    & ~~\wcard \mapsto \var{dataval} \in \var{hash_d}\restrictdom \fun{txdats}~{tx} \\
...
\begin{itemize}
  ...
  \item for each script input, the transaction is carrying the full
  validator script, and the full data value and redeemer (looked up by hash
...
\begin{figure}[htb]
  \begin{equation}\label{eq:utxo-inductive-shelley}
    ...
    { \fun{txlst}~tx \leq \var{slot}
      ...
      ~
      \\
      \forall~\var{(\var{txin},(\var{hash_d},\var{hash_r}))}\in\fun{txinputs_s}~{tx}, \\
      (\var{hash_d},\var{hash_r} \in \fun{dom}(\fun{txdats}~{tx})) ~\wedge \\
      ...
      \wedge ~\fun{hashData}~{dataval}~=~ \fun{getDataHash}~\var{txout})
      \\
      ~
      \\
