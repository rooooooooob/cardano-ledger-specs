...
\begin{figure}[htb]
  \begin{align*}
    ...
    & ~~ \fun{validationData}~\var{utxo}~\var{tx}~
      (txid,ix,(\var{dataval},\var{rdmr})))) ~\vert \\
...
\begin{figure}[htb]
  ...
  \begin{equation}
    ...
    {\fun{costm}~\var{pp},\var{remFee}} (\fun{snd}~{s})
    \\
    ~
    \\
  ...
  \begin{equation}
    ...
    {
    (\var{isVal},\var{remFee'})~:=~ \llbracket \fun{fst}~{s} \rrbracket {\fun{costm}~\var{pp},\var{remFee}} (\fun{snd}~{s})
    \\
    ~
    \\
