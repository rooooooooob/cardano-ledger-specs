...
\begin{figure}[htb]
  ...
  \begin{align*}
    ...
    \nextdef
    &\fun{findRdmrSet} \in \Tx \to \CurItem \to \powerset{\Data}\\
    & \text{get empty set or redeemer corresponding to index} \\
    & \fun{findRdmrSet}~{tx}~\var{it} ~=~ \{~ r ~\vert~ \\
    ...
    & \text{get redeemer corresponding to index or return Nothing} \\
    & \fun{findRdmr}~{tx}~\var{it} ~=~\begin{cases}
         \{\Nothing_d\} & \text{if~} \fun{findRdmrSet}~{tx}~\var{it}~=~\{\} \\
              \fun{findRdmrSet}~{tx}~\var{it}~ & \text{otherwise}
            \end{cases}
    \nextdef
...
\begin{figure}[htb]
  \begin{equation}\label{eq:utxo-inductive-shelley}
    ...
    { \fun{txlst}~tx \leq \var{slot}
      ...
      \\
      \fun{range}~(\fun{txinputs_{vf}}~{tx} \restrictdom \var{utxo})
      \subseteq \TxOutND \\
      ...
      ~
      \\
      \forall txout \in \txouts{tx}, q~ \in~ \{~q~\vert~ c\mapsto (t\mapsto q) \in \fun{getValue}~txout\} \\
      \Rightarrow q ~\geq ~0 \\
