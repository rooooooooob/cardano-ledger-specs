...
\begin{figure*}[htb]
  ...
  \begin{align*}
    ...
    &\text{returns the language tag, $\var{plcV1Tag}$ for Plutus V1} \\
    \nextdef
    \fun{plclangs} ~\in~& \powerset{\Script} \to \powerset{(\Language)} \\
    \fun{plclangs}~ \var{scrts} ~=~ & \{ \fun{language}~\var{scr} ~\vert~
      \var{scr}~\in~ \var{scrts}, \fun{language}~\var{scr} \in \{\var{plcV1Tag} \}  \}\\
...
\begin{figure*}[htb]
  ...
  \end{align*}
  \emph{Helper Script Validation Functions}
  %
  \begin{align*}
     &\fun{hashScript} \in  ~\Script\to \Language \\
     &\text{compute script hash} \\~\\
  \end{align*}
  %
  \emph{Notation}
