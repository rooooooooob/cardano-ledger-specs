...
\begin{itemize}
  ...
  have when spending a script output. This is the hashes of the redeemer and
  data values.
...
%      & \text{UTxO entry reference}
      \\
      \var{(\var{dvhash},\var{rdhash})}
      & \Info
      & \HashDat \times \HashDat
%      & \text{validation data}
      \\
...
\begin{itemize}
  ...
  \item $\TxBody$ also has the hash of the current protocol parameters, $\HashPP$
...
\begin{figure*}[htb]
  ...
  \begin{align*}
    ...
    & \powerset{\TxInTx} \times (\Ix \mapsto \TxOut) \times \seqof{\DCert}
      \times \Coin \times \Coin \times \Value \\
  ...
  \begin{equation*}
    \begin{array}{r@{~\in~}lr}
      ...
      \fun{txscrfee} & \Tx \to \Coin & \text{portion of fee allocated for scripts}\\
...
In order to perform the calculation which converts resource use into an
Ada price, the $\CostMod$ and $\Prices$ are also passed as parameters.
...
\begin{figure*}[htb]
  ...
  %
  \begin{align*}
    & \fun{valMSIGupTo} \in \CostMod \to \Prices \to\ScriptMSig\to \Data \to
    \Coin \to (\IsValidating \times \Coin) \\
    & \text{resource-restricted validation of a multi-sig script} \\~\\
    & \fun{valPLCupTo} \in \CostMod \to \Prices\to\ScriptPlutus\to
  ...
  %
  \begin{align*}
    \llbracket \var{script_v} \rrbracket_{\var{cm},\var{pr},\var{c}}(\var{dataval},~\var{rdmr},~\var{ptx})
    &=& \fun{valPLCupTo} ~{cm}~{pr}~\var{script_v}~(\var{dataval},~\var{rdmr},~\var{ptx},~
    \var{c})
    \nextdef
    \llbracket \var{script_v} \rrbracket_{\var{cm},\var{pr},\var{c}} \var{ptx}
    &=& \begin{cases}
    \fun{valPLCupTo} ~\var{cm}~{pr}~\var{script_v} ~ \var{\Nothing}~\var{\Nothing}~\var{ptx}~
    \var{c} ~& \text{if}~\var{script_v} \in \ScriptPlutus \\
    \fun{valMSIGupTo} ~\var{cm}~{pr}~\var{script_v} ~\var{ptx}~
