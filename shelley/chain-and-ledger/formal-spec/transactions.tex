...
%
  ...
  \begin{equation*}
    \begin{array}{r@{~\in~}l@{\qquad=\qquad}lr}
      ...
      \var{ptx}
      & \PendingTxInScript
      & \TxIn \times (\ScriptHash \times \ScriptHash) \times \Coin
...
\begin{figure*}[htb]
  ...
  \begin{equation*}
    \begin{array}{r@{~\in~}l@{\qquad=\qquad}l}
      ...
      \PendingTxInS \times \Slot \times \TxWitness
      \\
    \end{array}
  \end{equation*}
...
\begin{figure*}[htb]
  ...
  \begin{align*}
    ...
    & \begin{cases}
          ...
          (txin,c)
          & \var{txin} = (txid,ix) \in \TxIn
      \end{cases}\\
    & \text{build pending Tx input} \\~\\
    & \fun{pendingTxInScript} \in \TxInScr \to (\Coin \times \Addr) \to \PendingTxIn \\
    & \fun{pendingTxInScript}~(txin,(\var{script_v}, \var{script_r},\wcard))~\var{c} = \\
    &  (txin,\fun{hashScript}~{script_v},\fun{hashData}~{script_v},c)
    \\ & \text{build pending Tx script input}  \\~\\
    & \fun{validationData} \in \Tx \to \PendingTxInS \to \PendingTx \\
    & \fun{validationData}~\var{tx}~\var{ptxis} = \\ & (\{\fun{pendingTxIn}~\var{txin} \middle txin \in \fun{txins}~{tx}\},\{~\var{txout} \middle \var{ix} \mapsto \var{txout} \in \fun{txouts}~{tx}\},
    \fun{txfee}~{tx},\var{ptxis},\fun{txttl}~{tx},\fun{txwitsVKey}~{tx}) \\
    & \text{build PendingTx} \\
  \end{align*}
  %
  \emph{Notation}
