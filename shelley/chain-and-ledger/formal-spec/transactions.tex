...
\begin{itemize}
  ...
  the output the tag is attached to also contains the full datavalue corresponding
  ...
  \item $\TxOutND$: A transaction output with no datavalue hash
  (this type is for VK and multisig scripts outputs).

  \item $\TxOutP$: A transaction output type for Plutus
  script outputs (which include datavalue hash).
...
%      & \text {tag for inputs used to pay script fees}
      ...
      & \HasDV
      & \Yes \uniondistinct \Nope
%      & \text {tag for outputs that come with data values}
      \\
      \var{uin}
      & \UTxOIn
      & \TxId \times \Ix
...
\begin{itemize}
  ...
  \item a set of terms of type data, which contains all required datavalues
  \item an indexed structure containing $\Data$ terms, which includes all
  required redeemers
\end{itemize}

There is a difference between the way scripts and datavalues are included in
...
\begin{itemize}
  ...
  This is also the case with hashes of datavalues. A Plutus-script locked
  UTxO entry it contains the hash of the datavalue, and we can look for
  the datavalue with this hash in the set.
  ...
  , we pair the redeemer with a pointer to the item for
  which it will be used. This pointer is the key in the indexed redeemer structure.

  Recall that this pointer is  together with the item that requires a redeemer for validation,
  we put  redeemer structure is indexed by a pair of a tag indicating the type of
  thing being validated, and an index of it in the structure (list, set, or map)
  where it is stored. This is a reverse-pointer indexing approach.
...
There is no need to ever sign anything related to validator scripts of datavalues,
because a hash of every validator script that will be run during the validation
of the transaction is always part of the body, and the hash of every datavalue
...
\textbf{Processing Shelley Txs in the Goguen Era Transactions.}
Everything we have discussed so far in this section outlines the differences between Shelley era
and Goguen era transactions, which have different types. We refer to the Shelley
trasaction type as $\ShelleyTx$ and $\GoguenTx$ is the Goguen transaction.
For all other overlapping names of types used in both the Shelley and the Goguen
specifications, in this document, the names we use refer to the Goguen types only.
We later (in Section~\ref{sec:chain}) make a distinction between the Shelley and
Goguen chain states, as well as UTxOs, where we make the names explicit.

The reason we must make the distinction between the transaction types explicit from here onwards
is that in the Goguen era, we will be able to process both types of transactions
...
a Shelley transaction as a Goguen one. A crucial part of a Shelley transaction,
however, that cannot be transformed is the collection of signatures.

We will specify how we verify signatures before transforming and processing
a Shelley transction in the Goguen format when we discuss witnessing.
...
Given a set of
scripts, the function $\fun{cmlangs}$ returns a set of the languages that
require cost models to run the scripts,
see Figure \ref{fig:defs:functions-chain-helper}. To compute the
hash of the current parameters in the chain state,
(only the ones relevant to all those versions are selected and hashed by $\fun{hashLanguagePP}$.
...
the most common type of token on the ledger. More improtantly, it is the only
type of token relevant to all non-UTxO ledger accounting, including deposits,
...
The representation of Ada in the UTxO and inside transactions (as a term of the
$\Value$ type) is an implementation detail,
outside the scope of this specification. We make the following assumptions
about this representation:

\begin{itemize}
  \item the representation of Ada is unique
  \item the conversion of the $\Coin$ type to $\Value$ - it is implicit that
  the $\Value$ representation of Ada includes the special case for $\Coin$, and
  terms of these two types can be added or subtracted
  \item all Ada tokens are fungible with other Ada tokens (recall that this is
  not the case of all tokens a currency that is not Ada)
  \item there is no way to forge Ada tokens, as it it has a special representation
  with no corresponding forging script
\end{itemize}
...
\begin{figure*}[htb]
  ...
  \begin{align*}
     ...
     &\fun{validationData} \in  \UTxO \to \GoguenTx \to \CurItem \to \Data \\
