...
\begin{figure}[ht]
  \begin{equation}\label{eq:bbody}
    \inference[Block-Body]
    {
      ...
      \\~\\
      ((\var{utxo}, \wcard, \wcard, \wcard), \wcard) := ls \\
      \var{h_{pp}} := \fun{hash}~(\fun{getPlutusPP}~\var{pp}) \\
      \forall~tx\in txs,~\fun{mkScrLst}~\var{tx}~\var{utxo}~\neq~\epsilon
      \Rightarrow ~\fun{hashPP}~{tx} = \var{h_{pp}}
      \\~\\
      {
        {\begin{array}{c}
                 \bslot{bhb} \\
