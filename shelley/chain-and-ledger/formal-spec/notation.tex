...
\begin{description}
  ...
  $\type{FM}~\in~ \powerset(\type{X} \times \type{Y})$,
    ...
    \[\var{fm_x} = \{ y ~\vert~ (x, y)\in \var{fm} \} \]
    \[\sum_{(x, y)\in\var{fm}} (x,y) =
    \{ x\mapsto \sum_{y\in\var{fm}_x} y \} \]

    In the case $\type{Y}~=~\type{A}\mapsto\type{B}$ is itself a finite map,
    and addition is defined on terms of type $\type{B}$,
    we interpret
    \[\var{fm_{xa}} = \{ b ~\vert~
      (x, (a\mapsto b))\in \var{fm} \} \]
    \[\sum_{(x, (a\mapsto b))\in\var{fm}} (x,a\mapsto b) =
    \{ x \mapsto (a\mapsto \sum_{b\in\var{fm}_{xa}} b) \} \]

    We use the notation $+$ to denote this overloaded addition operation also.
